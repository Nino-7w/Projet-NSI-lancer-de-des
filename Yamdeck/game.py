...
class Game:
    ...
    def jouer_ia(self):
        ...
        # Stratégie : relancer si la somme est faible
        if sum(valeurs) < 20: 
    ...
    def draw(self, font):
        ...
        if self.gagnant_partie:
            ...
            self.screen.blit(msg, (larg//2 - msg.get_width()//2, 350))
            ...
            self.screen.blit(inst, (larg//2 - inst.get_width()//2, 400))
        elif self.attendre_ia:
            inst = font.render("ENTREE pour laisser jouer l'IA", True, (255, 165, 0))
            self.screen.blit(inst, (20, 440))
        elif self.current_player == 0 and not self.lancer_termine:
            inst = font.render("1,2,3: Select | R: Relancer | ENTREE: Valider", True, (0, 255, 0))
            self.screen.blit(inst, (20, 440))
